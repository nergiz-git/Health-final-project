...
function MedicationsPage() {
  ...
  // ➕ ADD - POST AddMedicationModal-də edilir, burda yalnız state update olunur
...
if (loading) {
  ...
  return (
    <div className="max-w-[1400px] mx-auto px-0 sm:px-0 lg:px-0 py-0 space-y-6">
      <motion.div
        initial={{ y: -80, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ duration: 0.6, ease: "easeOut" }}
          ...
          {medications.map((med) => (
            ...
            >
              <div className="flex flex-col sm:flex-row gap-4 items-start">
                ...
                <div className="space-y-1">
                  <h3 className="text-xl font-semibold text-slate-500">{med.name}</h3>
                  <p className="text-slate-500">
                    {med.dose} <span>mg</span>
                  </p>
