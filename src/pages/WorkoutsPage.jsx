...
// //       `${API_BASE_URL}/workouts/ai?weekStart=${weekStart}`,
// //       {
// //         method: "GET",
// //         headers: {
// //           Authorization: `Bearer ${token}`,
...
// //     // 1️⃣ AI-ni başlat (force=true)
// //     await fetch(
// //       `${API_BASE_URL}/workouts/ai?weekStart=${weekStart}&force=true`,
// //       {
// //         method: "GET",
// //         headers: {
// //           Authorization: `Bearer ${token}`,
...
// //     if (!res.ok) {
// //       throw new Error("AI plan hazır deyil. Yenidən cəhd edin.");
// //     }

// //     const data = await res.json();
// //     console.log("✅ AI WORKOUT PLAN:", data);
...
// //     // AI planı silmək üçün force parametri ilə GET göndər
// //     // backend-də force=true -> mövcud plan silinir
// //     await fetch(`${API_BASE_URL}/workouts/ai?weekStart=${weekStart}&force=true`, {
// //       method: "GET",
// //       headers: { Authorization: `Bearer ${token}` },
// //     });

// //     // fetchWorkouts ilə yenilənmiş planı gətir
// //     await fetchWorkouts();
...
// // };
// // const handleGenerate = async () => {
// //   if (!user) return alert("User məlumatları tapılmadı!");

// //   const confirmed = workouts.length > 0
// //     ? window.confirm("Mövcud məşq planı silinəcək. Yeni plan yaradılsın?")
// //     : true;
// //   if (!confirmed) return;
...
// //     if (!res.ok) throw new Error("AI plan yaradılmadı");
...
// //     // 1️⃣ AI-ni başlat (force=true) — cavabı gözləmirik
// //     await fetch(
// //       `${API_BASE_URL}/workouts/ai?weekStart=${weekStart}&force=true`,
// //       { headers: { Authorization: `Bearer ${token}` } }
// //     );

// //     // 2️⃣ Polling — hər 3 saniyədə bir yoxla, max 10 dəfə
// //     let attempts = 0;
...
// //         clearInterval(poll);
// //         setGenerating(false);
// //         alert("⚠️ AI planı hazırlamaq üçün vaxt lazımdır. Səhifəni yeniləyin.");
// //       }
// //     }, 3000);
...
// //         if (res.ok) {
// //           const data = await res.json();
// //           console.log(`POLL #${attempts}:`, data?.length, data?.[0]);
...
//     // ✅ 1. Əvvəlcə köhnə planı sil
//     if (workouts.length > 0) {
//       await Promise.all(
//         workouts.map((w) =>
//           fetch(`${API_BASE_URL}/workouts/${w.id}`, {
//             method: "DELETE",
//             headers: { Authorization: `Bearer ${token}` },
//           })
//         )
...
//     || workouts.find(w => w.dayOfWeek === daysAz[index])
//     || workouts[index];
//   return (
//     <TabsContent key={day} value={day} className="mt-0">
//       <motion.div
        ...
        <Card className="!bg-white backdrop-blur-sm border border-slate-200/80 !p-0 rounded-xl sm:rounded-2xl shadow-lg overflow-hidden">
          ...
          <div className="p-4 sm:p-7">
            <Tabs defaultValue={today} className="w-full">
              ...
              {daysEn.map((day, index) => {
  const dayWorkout = workouts[index]; // index əsaslı
